...
> & {
  ...
  canvasOrchestrationByWorkspaceKey?: AppState['canvasOrchestrationByWorkspaceKey']
}
  ...
  const payload = {
    ...
    defaultTerminalTabsAppliedByWorktreeId:
      ...
      Object.keys(snapshot.defaultTerminalTabsAppliedByWorktreeId).length > 0
        ? snapshot.defaultTerminalTabsAppliedByWorktreeId
        : undefined,
    canvasDocumentsByWorkspaceKey: Object.keys(snapshot.canvasDocumentsByWorkspaceKey ?? {}).length > 0 ? snapshot.canvasDocumentsByWorkspaceKey : undefined,
    canvasOrchestrationByWorkspaceKey: Object.keys(snapshot.canvasOrchestrationByWorkspaceKey ?? {}).length > 0 ? snapshot.canvasOrchestrationByWorkspaceKey : undefined,
