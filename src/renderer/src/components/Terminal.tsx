...
import {
  useCanvasBrowserPortals,
  type CanvasBrowserPortalTarget
} from './canvas/canvas-browser-portal'
...
function Terminal(): React.JSX.Element | null {
  ...
  const workspaceSurfaces = useMemo(() => {
    ...
    for (const portal of canvasTerminalPortals) {
      if (knownIds.has(portal.worktreeId)) {
        continue
      }
      surfaces.push({
        id: portal.worktreeId,
        path: activeWorktreeId
          ? (allWorktrees.find((worktree) => worktree.id === activeWorktreeId)?.path ?? '')
          : ''
      })
  ...
  return (
    ...
    >
      ...
      <TerminalPaneOverlayLayer
        ...
        shouldMeasureHiddenWorktree={shouldMeasureHiddenWorktree}
        activityTerminalPortals={activityTerminalPortals}
        canvasTerminalPortals={canvasTerminalPortals.filter(
          (portal) => portal.worktreeId === worktreeId
        )}
      ...
      {isVisible || backgroundMountTabIds === null ? (
        <>
          <BrowserPaneOverlayLayer
            worktreeId={worktreeId}
            isWorktreeActive={
              isVisible || canvasBrowserPortals.some((portal) => portal.worktreeId === worktreeId)
            }
            canvasBrowserPortals={canvasBrowserPortals.filter(
              (portal) => portal.worktreeId === worktreeId
            )}
          />
