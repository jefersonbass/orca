...
test.describe('Spatial Canvas', () => {
  ...
  test('creates and persists a note while isolating workspaces', async ({ orcaPage }) => {
    ...
    if (!canvasBox) throw new Error('Canvas surface is not measurable')
    ...
    await expect.poll(() => orcaPage.evaluate(() => window.__store?.getState().canvasDocument?.nodes.find((node) => node.type === 'agent-terminal')?.resourceRef?.kind)).toBe('terminal-tab')
    await orcaPage.getByRole('button', { name: 'Fit view', exact: true }).click()
    ...
    if (!agentBox) throw new Error('Agent target is not measurable')
    ...
    const zoomButtonBackground = await orcaPage.getByRole('button', { name: 'Zoom in' }).evaluate((element) => getComputedStyle(element).backgroundColor)
  ...
  test('marquee-selects multiple nodes and drags them as one group', async ({ orcaPage }) => {
    ...
    if (!paneBox || !boxes[0] || !boxes[1]) throw new Error('Canvas nodes are not measurable')
    ...
    const endX = Math.min(paneBox.x + paneBox.width - 8, Math.max(boxes[0].x + boxes[0].width, boxes[1].x + boxes[1].width) + 24)
    const endY = Math.min(paneBox.y + paneBox.height - 8, Math.max(boxes[0].y + boxes[0].height, boxes[1].y + boxes[1].height) + 24)
    ...
    const before = await orcaPage.evaluate(() => window.__store?.getState().canvasDocument?.nodes.map((node) => ({ id: node.id, ...node.position })))
    const firstBox = await nodes.nth(0).boundingBox()
    if (!firstBox) throw new Error('Selected source node is not measurable')
    ...
    await expect.poll(async () => orcaPage.evaluate(() => window.__store?.getState().canvasDocument?.nodes.map((node) => ({ id: node.id, ...node.position })))).not.toEqual(before)
    const after = await orcaPage.evaluate(() => window.__store?.getState().canvasDocument?.nodes.map((node) => ({ id: node.id, ...node.position })))
  ...
  test('resizes from the pointer position, persists size, and exits edit mode with Escape', async ({ orcaPage }) => {
    await seedInteractionCanvas(orcaPage)
    ...
    const handleBoxes = await handles.evaluateAll((elements) => elements.map((element, index) => {
      const rect = element.getBoundingClientRect()
      return { index, x: rect.x, y: rect.y, width: rect.width, height: rect.height }
    }))
    const bottomRight = handleBoxes.reduce((best, current) => current.x + current.y > best.x + best.y ? current : best)
    const before = await orcaPage.evaluate(() => window.__store?.getState().canvasDocument?.nodes.find((node) => node.id === 'interaction-source')?.size)
    await orcaPage.mouse.move(bottomRight.x + bottomRight.width / 2, bottomRight.y + bottomRight.height / 2)
    await orcaPage.mouse.down()
    await orcaPage.mouse.move(bottomRight.x + bottomRight.width / 2 + 90, bottomRight.y + bottomRight.height / 2 + 70, { steps: 12 })
    await orcaPage.mouse.up()

    await expect.poll(async () => orcaPage.evaluate(() => window.__store?.getState().canvasDocument?.nodes.find((node) => node.id === 'interaction-source')?.size.width)).toBeGreaterThan(before?.width ?? 0)
    const after = await orcaPage.evaluate(() => window.__store?.getState().canvasDocument?.nodes.find((node) => node.id === 'interaction-source')?.size)
    expect(after?.height).toBeGreaterThan(before?.height ?? 0)
    await orcaPage.keyboard.press('Escape')
    await expect(handles).toHaveCount(0)
  })

  test('anchors the connection preview to the pointer and creates exactly one edge', async ({ orcaPage }) => {
    await seedInteractionCanvas(orcaPage)
    ...
    if (!captureBox) throw new Error('Canvas connection layer is not visible')
    ...
    const firstPointer = await pointer.evaluate((element) => ({ cx: element.getAttribute('cx'), cy: element.getAttribute('cy') }))
    await capture.hover({ position: { x: captureBox.width / 2 + 120, y: captureBox.height - 220 } })
    await expect.poll(async () => pointer.evaluate((element) => ({ cx: element.getAttribute('cx'), cy: element.getAttribute('cy') }))).not.toEqual(firstPointer)
    ...
    if (!targetBox) throw new Error('Connection target is not measurable')
    await orcaPage.mouse.click(targetBox.x + targetBox.width / 2, targetBox.y + targetBox.height / 2)
    await expect(orcaPage.locator('.canvas-edge-path')).toHaveCount(1)
  ...
  test('records a real freehand path and edits shape text and label size', async ({ orcaPage }) => {
    await orcaPage.evaluate(() => window.__store?.getState().setCanvasDocument({ version: 2, viewport: { x: 0, y: 0, zoom: 1 }, nodes: [], edges: [] }))
    const pane = orcaPage.locator('.react-flow__pane')
    const paneBox = await pane.boundingBox()
    if (!paneBox) throw new Error('Canvas pane is not visible')
    ...
    const freehandPoints = [[140, 150], [180, 120], [225, 175], [270, 105], [330, 160]]
    await orcaPage.mouse.move(paneBox.x + freehandPoints[0][0], paneBox.y + freehandPoints[0][1])
    await orcaPage.mouse.down()
    for (const [x, y] of freehandPoints.slice(1)) await orcaPage.mouse.move(paneBox.x + x, paneBox.y + y, { steps: 4 })
    await orcaPage.mouse.up()
    await expect(orcaPage.locator('[aria-label="freehand shape"]')).toHaveCount(1)
    const recordedPoints = await orcaPage.evaluate(() => window.__store?.getState().canvasDocument?.nodes.find((node) => node.type === 'drawing')?.metadata?.points)
    ...
    await expect.poll(() => orcaPage.evaluate(() => window.__store?.getState().canvasDocument?.nodes.find((node) => node.type === 'rectangle')?.label)).toBe('Decision gate')
    ...
    await expect.poll(() => orcaPage.evaluate(() => window.__store?.getState().canvasDocument?.nodes.find((node) => node.type === 'label')?.metadata?.fontSize)).toBe(24)
  ...
  test('exports non-empty SVG and PNG snapshots of the rendered canvas', async ({ orcaPage }) => {
    ...
    await orcaPage.evaluate(() => {
      ...
      ;(window as any).__canvasExportDownloads = downloads
    ...
    await expect.poll(() => orcaPage.evaluate(() => (window as any).__canvasExportDownloads?.length)).toBe(1)
    const svg = await orcaPage.evaluate(async () => {
      const item = (window as any).__canvasExportDownloads[0]
    ...
    await expect.poll(() => orcaPage.evaluate(() => (window as any).__canvasExportDownloads?.length)).toBe(2)
    const png = await orcaPage.evaluate(async () => {
      const item = (window as any).__canvasExportDownloads[1]
      const response = await fetch(item.href)
  ...
  test('groups dropped nodes into a frame and moves the frame contents together', async ({ orcaPage }) => {
    await orcaPage.evaluate(() => window.__store?.getState().setCanvasDocument({
      version: 2,
      viewport: { x: 0, y: 0, zoom: 1 },
      nodes: [
        { id: 'frame-e2e', type: 'group', position: { x: 80, y: 80 }, size: { width: 480, height: 300 }, zIndex: 1, label: 'Workflow' },
        { id: 'frame-note-e2e', type: 'note', position: { x: 650, y: 120 }, size: { width: 220, height: 140 }, zIndex: 2, label: 'Frame note', metadata: { content: 'Move me' } },
      ],
      edges: [],
    }))
    ...
    if (!frameBox || !noteBox) throw new Error('Frame test nodes are not measurable')
    await orcaPage.mouse.move(noteBox.x + noteBox.width / 2, noteBox.y + 20)
    await orcaPage.mouse.down()
    await orcaPage.mouse.move(frameBox.x + frameBox.width / 2, frameBox.y + frameBox.height / 2, { steps: 12 })
    await orcaPage.mouse.up()
    await expect.poll(() => orcaPage.evaluate(() => window.__store?.getState().canvasDocument?.nodes.find((node) => node.id === 'frame-note-e2e')?.groupId)).toBe('frame-e2e')

    const before = await orcaPage.evaluate(() => window.__store?.getState().canvasDocument?.nodes.map((node) => ({ id: node.id, ...node.position })))
    const movedFrameBox = await frame.boundingBox()
    if (!movedFrameBox) throw new Error('Frame is not measurable after grouping')
